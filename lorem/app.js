...
var result = document.querySelector('.lorem-text');
...
form.addEventListener('submit', function(e) {
    ...
    if (isNaN(value) || value < 0 || value > 10) {
        result.innerHTML = `<p class="result" style="width: 53%; margin: auto; margin-top: 32px;">${text[random]}</p>`;
    } else {
        let temText = text.slice(0, value);
        temText = temText.map(function(item) {
            return `<p class="result" style="width: 53%; margin: auto; margin-top: 32px;">${item}</p>`;
        }).join("");
        result.innerHTML = temText;
        result.style.cssText = 'width: 100%; margin: auto; margin-top: 22px;';
    }
});
